...
## 开发流程：Traffic Light Development

为了确保生信分析流程的稳健性，Agent 在开发过程中应执行以下循环：

- **Step [RED]**: 定义期望。先写测试逻辑（或断言），明确数据输入与输出的规格。
- **Step [GREEN]**: 快速实现。完成功能逻辑，使自动化流水线或单元测试点亮绿灯。
- **Step [REFACTOR]**: 质量提升。重构代码以符合 Clean Code 标准，并补充必要的文档注释。

> **Note**: 严禁跳过 [RED] 阶段直接交付功能。所有的 Feature 开发必须伴随对应的 Test Case。
