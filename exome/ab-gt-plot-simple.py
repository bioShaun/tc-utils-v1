...
COLOR_MAP = {"HET": "#E41A1C", "REF": "#0987ed", "ALT": "#0ec950"}
...
def plot_origin(
    ...
    female_patch = mpatches.Patch(color="#0987ed", label="REF")
    male_patch = mpatches.Patch(color="#0ec950", label="ALT")
    both_patch = mpatches.Patch(color="#E41A1C", label="HET")
...
def main(
    ...
    sample_list = gt_df.columns[4:]
    for child_name in tqdm(sample_list):
        child_df = gt_df[["CHROM", "POS", child_name]]
        child_df[child_name] = child_df[child_name].map(map_gt)
        child_df = child_df[child_df[child_name] != "MISS"]
        plot_origin(chr_df, child_df, out_dir, child_name)
