...
def main(
    r1_paf: Path, r2_paf: Path, probe_fa: Path, count_file: Path, threads: int = 4
) -> None:
    if count_file.is_file():
        ...
        probe_list = [
            each.strip().replace(">", "")
            for each in open(probe_fa)
            if each.startswith(">")
        ]
        probe_df = pd.DataFrame(probe_list, columns=["probe"])
        pandarallel.initialize(progress_bar=True, nb_workers=threads)
        r1_df = pd.read_csv(
            r1_paf, usecols=[0, 5], header=None, names=["read_name", "probe"], sep="\t"
        )
        r2_df = pd.read_csv(
            r2_paf, usecols=[0, 5], header=None, names=["read_name", "probe"], sep="\t"
        )
        r1_df["read_prefix"] = r1_df["read_name"].parallel_map(
            lambda x: x.split("/")[0]
        )
        r2_df["read_prefix"] = r2_df["read_name"].parallel_map(
            lambda x: x.split("/")[0]
        )
