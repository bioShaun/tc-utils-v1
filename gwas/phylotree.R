...
if (tree$Nnode <= 50) {
  linear_tree <- ggtree(tree, branch.length="none", ladderize = FALSE) + geom_tiplab(size=text_size) + geom_rootedge()
  original_x_xmax <- max(linear_tree$data$x)
  linear_x_max = original_x_xmax + original_x_xmax * 2 / (linear_width - 2)
  linear_tree <- ggtree(tree, branch.length="none", ladderize = FALSE) + geom_tiplab(size=text_size) + geom_rootedge() + xlim(0, linear_x_max)
...
circular_tree <- ggtree(tree, branch.length="none", layout='circular', ladderize = FALSE) + geom_tiplab(size=text_size) + geom_rootedge()
