...
def parse_tmap_pandas(tmap_file, valid_classes={'=', 'c', 'k', 'm', 'j'}):
    ...
    if 'ref_gene_id' in header and 'class_code' in header and 'qry_gene_id' in header:
        ...
        df = pd.read_csv(tmap_file, sep='\t', usecols=use_cols, na_values=['-'], comment='#')
    else:
        # 如果header不规范，则按位置读取，并假设没有header
        # 标准tmap格式: 列1是ref_gene_id, 列3是class_code, 列5是qry_gene_id
        try:
            df = pd.read_csv(tmap_file, sep='\t', header=None, na_values=['-'], comment='#')
            # 提取需要的列
            df = df[[0, 2, 4]]
    ...
    df.drop_duplicates(subset=['qry_gene_id', 'ref_gene_id'], keep='first', inplace=True)
...
def main():
    ...
    parser.add_argument('-o', '--output', default='gene_id_map.txt', help='输出文件(默认: gene_id_map.txt)')
    parser.add_argument(
        '-c', '--classes',
        default='=,c,k,m,j',
