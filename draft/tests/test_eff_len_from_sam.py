...
from draft.eff_len_from_sam import compute_effective_lengths, parse_md_mismatches

# 测试 SAM 内容，同时包含 XM 和 MD
SAM_CONTENT = """\
@SQ SN:chr1 LN:1000
...
def test_compute_effective_lengths(tmp_sam):
    ...
    # read1: CIGAR 10M, XM=2 -> eff_xm=8, MD mismatch=2 -> eff_md=8
    assert compute_effective_lengths(reads[0]) == (8, 8)
    # read2: CIGAR 5M5M=10, XM=1 -> eff_xm=9, MD mismatch=2 -> eff_md=8
