...
def create_dual_bar_chart(
    ...
    width = 0.35
    rects1 = ax1.bar(
        x - width / 2, df["Probe_Count"], width, label="探针数量", color="#4472C4"
    )
    ax1.set_ylabel("探针数量", fontsize=12)
    ax1.set_xlabel("染色体", fontsize=12)
    ...
    ax2.set_ylabel("染色体长度 (Mb)", fontsize=12)
    ...
    # 添加标题
    plt.title("探针数量与染色体长度", fontsize=14, pad=20)
